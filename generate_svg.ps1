...
$xmSvg = New-Svg @(
  ...
  '  <rect x="30" y="160" width="535" height="150" rx="10" fill="url(#xs1)" stroke="#A5D6A7" stroke-width="1.5"/>',
  ...
  '  <text x="200" y="255" font-size="50" fill="#333" text-anchor="middle" font-family="monospace" font-weight="bold">' + $xmLetterUp + '</text>',
  '  <text x="280" y="255" font-size="50" fill="#CFD8DC" text-anchor="middle" font-family="monospace" font-weight="bold">' + $xmLetterUp + ' ' + $xmLetterUp + ' ' + $xmLetterUp + ' ' + $xmLetterUp + '</text>',
  '  <text x="200" y="295" font-size="50" fill="#333" text-anchor="middle" font-family="monospace" font-weight="bold">' + $xmLetterLow + '</text>',
  '  <text x="280" y="295" font-size="50" fill="#CFD8DC" text-anchor="middle" font-family="monospace" font-weight="bold">' + $xmLetterLow + ' ' + $xmLetterLow + ' ' + $xmLetterLow + ' ' + $xmLetterLow + '</text>',
  '  <rect x="30" y="325" width="535" height="90" rx="10" fill="url(#xs2)" stroke="#F48FB1" stroke-width="1.5"/>',
  '  <text x="297" y="353" font-size="17" font-weight="bold" fill="#C62828" text-anchor="middle">&#x1F50D; 找字母</text>',
  '  <text x="297" y="370" font-size="13" fill="#78909C" text-anchor="middle">把字母 ' + $XMLetter.Substring(0,1) + ' 圈出来：</text>',
  '  <text x="297" y="405" font-size="28" fill="#333" text-anchor="middle" font-family="monospace">a a a  a a a  b c d e f g h</text>',
  '  <rect x="30" y="430" width="535" height="100" rx="10" fill="url(#xs3)" stroke="#9FA8DA" stroke-width="1.5"/>',
  '  <text x="297" y="458" font-size="17" font-weight="bold" fill="#283593" text-anchor="middle">&#x1F4E3; 单词认读</text>',
  '  <text x="297" y="475" font-size="13" fill="#78909C" text-anchor="middle">大声读出下面的单词：</text>',
  '  <text x="297" y="518" font-size="22" fill="#333" text-anchor="middle" font-weight="bold">' + $XMWords + '</text>',
  '  <rect x="30" y="545" width="535" height="155" rx="10" fill="url(#xs2)" stroke="#F48FB1" stroke-width="1.5"/>',
  '  <text x="297" y="573" font-size="17" font-weight="bold" fill="#C62828" text-anchor="middle">&#x1F399;&#xFE0F; 发音练习 · 首字母发音</text>',
  '  <text x="297" y="590" font-size="13" fill="#78909C" text-anchor="middle">大声说出每个单词的首字母发音，再读整词：</text>',
  '  <text x="297" y="625" font-size="20" fill="#333" text-anchor="middle">' + $xmWordsList[0] + ' → /' + $XMLetter.Substring(0,1).ToLower() + '/</text>',
  '  <text x="297" y="655" font-size="20" fill="#333" text-anchor="middle">' + $xmWordsList[1] + ' → /' + $XMLetter.Substring(0,1).ToLower() + '/</text>',
  '  <text x="297" y="685" font-size="20" fill="#333" text-anchor="middle">' + $xmWordsList[2] + ' → /' + $XMLetter.Substring(0,1).ToLower() + '/</text>',
  '  <text x="297" y="745" font-size="12" fill="#B0BEC5" text-anchor="middle">生成日期：' + $DateStr + '</text>',
  '</svg>'
)
...
$cnSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 595 1000" width="595" height="1000">
  ...
  <rect x="30" y="160" width="535" height="200" rx="10" fill="url(#cs1)" stroke="#FFCC80" stroke-width="1.5"/>
  <text x="297" y="188" font-size="17" font-weight="bold" fill="#E65100" text-anchor="middle">&#x1F3A4; 拼音跟读 + 写一写</text>
  <text x="297" y="240" font-size="26" fill="#333" text-anchor="middle" font-weight="bold">' + $ChinesePinyin + '</text>
'
...
foreach ($py in $pyList) {
  if ($pyY -gt 350) { break }
  $cnSvg += "  <text x='297' y='${pyY}' font-size='20' fill='#CFD8DC' text-anchor='middle' font-family='monospace' font-weight='bold'>$py  $py  $py  $py  $py</text>"
  $pyY += 24
}
# Tone syllables
if ($pyList.Count -le 3) {
  ...
  if ($syllabes.Count -gt 0) { $cnSvg += "  <text x='297' y='${pyY}' font-size='18' fill='#FF8A65' text-anchor='middle'>" + ($syllabes -join '  ') + '</text>' }
}

$cnSvg += @"
  <rect x="30" y="380" width="535" height="220" rx="10" fill="url(#cs2)" stroke="#FFAB91" stroke-width="1.5"/>
  <text x="297" y="408" font-size="17" font-weight="bold" fill="#BF360C" text-anchor="middle">&#x270F;&#xFE0F; 汉字描红 · 每个字描3遍</text>
"@

$yPos = 445
foreach ($c in $cnCharList) {
  $cnSvg += "  <text x='297' y='${yPos}' font-size='30' fill='#CFD8DC' text-anchor='middle' font-family='STKaiti,serif' font-weight='bold'>$c  $c  $c  $c  $c</text>"
  $yPos += 42
}
...
if ($ChinesePoem1Title) {
  $cnSvg += '  <text x="297" y="' + $pY + '" font-size="15" font-weight="bold" fill="#283593" text-anchor="middle">&#x1F4DA; ' + $ChinesePoem1Title + ' — ' + $ChinesePoem1Author + '</text>'
  $pY += 24
  $p1Lines = Format-PoemLines $ChinesePoem1Text
  foreach ($l in $p1Lines) {
    $cnSvg += '  <text x="80" y="' + $pY + '" font-size="16" fill="#333" font-family="STKaiti,serif">' + $l + '</text>'
    $pY += 22
  }
  $pY += 10
}
if ($ChinesePoem2Title) {
  $cnSvg += '  <text x="297" y="' + $pY + '" font-size="15" font-weight="bold" fill="#283593" text-anchor="middle">&#x1F4DA; ' + $ChinesePoem2Title + ' — ' + $ChinesePoem2Author + '</text>'
  $pY += 24
...
$taskY = $yPos + $pBoxH + 20
$cnSvg += '  <rect x="30" y="' + $taskY + '" width="535" height="55" rx="10" fill="#FFCC02" opacity="0.2"/>
  <text x="297" y="' + ($taskY + 33) + '" font-size="15" fill="#E65100" text-anchor="middle">&#x1F3C6; 今日任务：跟读3遍 | 认读 | 描红 | 读古诗 | 在家找字</text>
  <text x="297" y="' + ($taskY + 65) + '" font-size="12" fill="#B0BEC5" text-anchor="middle">活动：' + $ChineseAct + '</text>
  <text x="297" y="' + ($taskY + 95) + '" font-size="12" fill="#B0BEC5" text-anchor="middle">生成日期：' + $DateStr + '</text>
</svg>'
